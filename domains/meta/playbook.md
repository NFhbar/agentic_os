...
- `meta-mine-sessions` — mine session transcripts for automation candidates: clusters interactive turns by normalized intent (tool/file digests included), classifies the top clusters (skill / schedule / orchestrator-step / dispatched-echo / inherently-manual), writes a ranked spend-attached report to `vault/output/meta/automation-candidates/`. Weekly via [[runbook-weekly-session-mining]].
...
- `meta-eval-skill-edit` — replay-eval a proposed SKILL.md edit before the decision gate (v1: dev-pr-review only). Replays 1-3 stored historical review prompts against git-pinned repo state with the edited skill inlined (`scripts/eval-skill-edit.mjs` reconstructs each pass's head + historic merge-base), then judges old vs new output on the audit-rubric dimensions: better / same / worse + rationale to `vault/output/meta/tuning-evals/`. The minutes-scale regression check that closes the open-loop gap between skill edits and ~weekly lifecycle audits.
- `meta-review-project-plan` — read-only peer-review of a project plan; writes a structured verdict (approve / request-changes / reject) and flips the shared `review_status` accordingly (`plan_status` stays `drafted`)
- `meta-revise-project-plan` — folds review findings back into the project plan in place; bumps `plan_revision`, resets `review_status: pending` so the revised plan returns to review
- `meta-scaffold-project-plan` — terminal phase; gated on `review_status: approved`; dispatches `dev-add-change` / `meta-add-schedule` / direct frontmatter edits to materialize the approved plan's items
