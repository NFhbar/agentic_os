...
- `dev-pr-review-publish` — publishes a pr-review entry's pass back to GitHub via the `github` MCP. Partitions the publish set first: comments answering an existing thread go out as threaded replies (`mcp__github__reply_to_pull_request_comment`), fresh findings go out as one batched review with a verdict (`APPROVE` / `REQUEST_CHANGES` / `COMMENT`) mapped deterministically from the entry's `result` field. A replies-only publish posts no review event at all — answering an author must not re-request changes. Accepted-only filter (dismissed/new comments are skipped); idempotent on already-published comments. Writes `github_comment_id` (+ `github_review_id` for reviewed comments) + `status: published` back to the entry, so replies can be threaded against on the next ingest. Used in the external-PR flow where the OS reviewed a non-OS PR.
...
- `dev-pull-pr-comments` — inbound counterpart of `dev-pr-review-publish`. Calls `mcp__github__list_pull_request_reviews` + `mcp__github__list_pull_request_review_comments` to ingest external reviewers' comments, author replies, and thread replies into the linked pr-review entry as a new pass (marked `agent: external` in the pass config). Each comment carries `github_comment_id` + `github_review_id` + `author` upfront for idempotent re-runs; replies additionally carry `in_reply_to` naming their parent (`pass-N-comment-M` when the entry already holds it, else `github:<id>`). After ingest, external comments flow through the same Accept/Dismiss/Re-implement loop as model-generated ones, and unanswered ones feed `dev-pr-review`'s response mode.
- `dev-drive-project` — project-tier driver: sequences a project's scaffolded changes through the lifecycle in `parent_change` dependency order, deriving each change's state fresh from the vault on every gate (mixed-mode safe, no locks, no driver-only state). Dispatches each node through the same endpoints the dashboard buttons call, stamping `origin: driver`; delegates execute → open-PR → PR-review to the change-automation orchestrator via enable+start; verifies each step's artifact rather than its exit code. Happy path only — any park, failure, or ambiguity is a clean hand-back to the operator, and every human gate (draft acceptance, plan-review judgment, comment triage, Mark ready, PR merge) stops the drive. Never self-engaging: explicit invocation only.
