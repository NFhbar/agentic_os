...
3. **Validate the gate** — read the change fields, and when `pr_review_path` is set, the linked pr-review entry:
   ...
   | `pr_review_status !== 'ready-for-human'` | **idempotent stop** (not an error): `Change \`<change>\` is already marked ready-for-human (since <pr_ready_at>). Nothing to do.`Skip steps 4–5; still record an event with`noop: true`. Checked BEFORE the untriaged-comments gate below, so a change already marked ready (e.g. via override) re-runs as the documented no-op even if its latest pass still carries `new` comments.                                         |
   ...
   Treat `override: true` as a permission slip — it skips the pr_review_status-present, needs-changes, and untriaged-comments checks (never the idempotent stop) but is recorded verbatim in the event payload. Reading the linked pr-review entry for the untriaged-comments count is allowed; writing to it remains forbidden (see "What this skill must NOT do").
