...
| `close project`, `complete project`, `abandon project`, `cancel project`                   | `meta-close-project`                |
...
| `draft response`, `answer pr comments`, `respond to reviewer`, `reply to pr comments`      | `dev-pr-review` (`mode: response`)  |
...
| `.claude/state/runs.db`                    | Run ledger (SQLite) for dashboard / driver / scheduler dispatches — status, pid, `origin` (`human`/`automation`/`scheduler`/`driver`; vocabulary in `scripts/run-origins.mjs`), model/effort overrides. Queued rows with no pid and no journal are reaped after a 2-minute grace. |
| `.claude/state/runs/`                      | Per-run journals (`r_<id>.raw.jsonl`) + stderr sidecars (`r_<id>.stderr.log`), tail-read by the run supervisor and the model-availability classifier.                                                                                                                             |
| `.claude/state/supervision-heartbeat.json` | Supervision heartbeat — stamped by every run-supervision host (`scripts/runs-supervisor.mjs` and the in-process close handler); watched by the `supervision-stale` audit check (`scripts/audit-supervision.mjs`), which escalates when runs are in flight.                        |
