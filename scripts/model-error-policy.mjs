...
// Shared real-YAML parser. The reference spec reads these three keys with a
// flat line regex; this repo consolidated every runtime frontmatter read onto
// one parser (see scripts/frontmatter.mjs), and that parser is a superset of
// the flat form — it accepts the documented scalar shape AND a YAML list of
...
//
// The loop guard (§8.6: "the fallback hook fires once") has two arms: the
// spec's — the run already resolved to the declared fallback — and a title
