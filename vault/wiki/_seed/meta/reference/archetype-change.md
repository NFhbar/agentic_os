...
| field              | type    | notes                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pr_review_status` | enum    | `pending` (no review yet, OR approving verdict with a standing blocker/bug comment — legacy "review ran, undistinguished"), `approved` (latest pass approved with no standing blocker/bug — awaiting human comment triage + Mark ready), `needs-changes` (latest pass's result was `request-changes`), `ready-for-human` (user clicked Mark ready — set by `dev-mark-pr-ready`; vault-only, no GitHub side-effects) |
...
- `dev-mark-pr-ready` flips `pr_review_status: ready-for-human` and sets `pr_ready_at`. It accepts `pending` _or_ `approved` as the normal prior status, and refuses (override available) while the linked review's latest pass has `status: new` comments. Vault-only — no GitHub calls. The user reviews and merges the PR on GitHub themselves.
