...
- quote: if (!token) return null;             ← the code the comment is about
- status: new
- prior: <comment-id-from-previous-pass>      ← only set on re-reviews
- author: <github-login>                      ← only on a comment a person wrote
- in_reply_to: <parent-ref>                   ← only on a comment answering another comment
- body_source: operator                       ← only when a person wrote the body

<comment body — free markdown, can include code blocks + suggestions>
```

The optional `start_line` / `side` / `start_side` / `quote` lines are emitted between `- line:` and `- status:` (single-line RIGHT-side comments with no quote omit all four). A parser reading these headers MUST recognize them as header keys so an unrecognized line never leaks `- status:` into the message body. The same applies to every header key below, and to keys added later — treat any leading `- key: value` line as a header, not as body text.
...
| `quote`             | string                                                                                              | Optional but strongly preferred on any anchored comment. The exact code text the comment is about, copied off the diff at `line` (or spanning `start_line`..`line`), written bare on one line — no wrapping backticks, and a multi-line span joined with spaces. It turns the anchor from a claim about position into a claim about content: `scripts/annotate-diff-lines.mjs --validate` confirms the text is at the claimed lines, moves the anchor to where the text actually is, or refuses the anchor entirely when the text is nowhere in the diff. Comparison is whitespace-normalized, so indentation and re-wrapping never break a match. Omit only when there is no code to quote (file-level comments) or the comment is a reply.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `author`            | GitHub login                                                                                        | Set by `dev-pull-pr-comments` on every ingested comment. Attribution belongs in the header, never wrapped around the body as prose — the body stays the reviewer's own words. Absent means the model wrote it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `in_reply_to`       | `pass-<N>-comment-<M>` or `github:<id>`                                                             | Present only on a comment that answers another comment. Two forms, no others: `pass-<N>-comment-<M>` when the parent is a comment this entry holds (which carries the `github_comment_id` naming the GitHub thread), `github:<id>` when the parent lives only on GitHub. Written by `dev-pull-pr-comments` on ingested replies and by `dev-pr-review`'s response mode on drafted replies; read by `dev-pr-review-publish` to send the comment as a threaded reply rather than a fresh finding.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `body_source`       | `operator`                                                                                          | Present only when a person wrote the body. `dev-pr-review-publish` publishes such a body byte-for-byte — no drift marker, no accept-note footer, no framing of any kind. Absent (or any other value) means model-generated, which keeps the marker treatment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- file: `src/auth/login.ts`
- line: 42
- quote: const token = req.headers.authorization;
- status: new
...
The `### Pass config` block is a header list like a comment's, and it records who produced the pass and under what settings:

| key            | values                           | notes                                                                                                                                                                                                                                            |
| -------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `model`        | model id                         | The model that produced a model-generated pass. Absent on ingested passes — no model ran.                                                                                                                                                        |
| `agent`        | `external`                       | Marks a pass whose comments were written by people, ingested by `dev-pull-pr-comments`, rather than produced by the model. Absent means model-generated. This is the single marker consumers key off to render and reason about a pass's origin. |
| `mode`         | `response`                       | Marks a pass of drafted replies from `dev-pr-review`'s response mode. Such a pass carries no verdict — it pairs with `result: suppressed — response pass carries no verdict` and leaves the entry's `result` frontmatter untouched.              |
| `reviewers`    | comma-separated GitHub logins    | On an ingested pass, who commented.                                                                                                                                                                                                              |
| `focus areas`  | comma-separated categories       | Config snapshot for a review pass.                                                                                                                                                                                                               |
| `style`        | `terse` / `concise` / `detailed` | Config snapshot for a review pass.                                                                                                                                                                                                               |
| `code context` | free text                        | What the review could actually read (PR-head worktree, base cache, diff-only).                                                                                                                                                                   |
| `commit_id`    | SHA                              | The commit the pass anchors to.                                                                                                                                                                                                                  |
...
- file: `LICENSE`
- line: 3
- quote: Copyright (c) 2025 Acme, Inc.
- status: new
