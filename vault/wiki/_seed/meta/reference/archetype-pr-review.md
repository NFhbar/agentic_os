...
The optional `start_line` / `side` / `start_side` lines are emitted between `- line:` and `- status:` (single-line RIGHT-side comments omit all three). A parser reading these headers MUST recognize them as header keys so an unrecognized line never leaks `- status:` into the message body. The same applies to every header key below, and to keys added later — treat any leading `- key: value` line as a header, not as body text.
