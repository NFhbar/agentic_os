...
| `run-origin-missing`               | error/warn/info | Every `runs` row carries a valid `origin` (`human` \| `automation` \| `scheduler` \| `driver`). The `origin` column must exist (error if missing); NULL rows are legacy and tolerated (info — read as `human`); a non-NULL value outside the vocabulary is an error (a dispatch path stamped an unknown origin). `warn` when other expected `runs` columns are absent.         | `standard-event-store` |
