...
| `note-run-telemetry`                | warn     | No `type: note` entry restates run telemetry: a `.jsonl` source AND a date-bucketed id together mark an event-log restatement (Finding 5.2; OS.md layer contract — telemetry lives in events.db/raw, analysis lives in the wiki).                                      | `meta-curate` § telemetry rule |
...
| `tuning-target-path-missing` | error    | Every path in `scripts/tuning-targets.mjs` TUNING_TARGETS exists on disk. Map rot silently reverts non-skill Overseer suggestions to rationale-only dead ends (Finding 3.2).                                   | `tuning-targets.mjs`     |
...
| `events-skill-attribution-missing` | warn       | Events whose `skill` is one of the change-scoped skills (`dev-write-change`, `dev-review-change`, `dev-open-pr`, `dev-close-change`, `dev-pr-review`, `dev-add-change`, `dev-address-comments`) MUST have `change_id` set. Catches drift in `recordEvent` callers that forget to lift the change id out of the prompt/intent/path via `scripts/extract-event-attribution.mjs`. Rows that carry no attribution by construction are excluded — `ai-prompt` / `router`-`route` envelopes and explicit-null `args` (`scripts/audit-attribution-scope.mjs`). | `standard-event-store` |
| `run-origin-missing`               | error/info | Every `runs` row carries a valid `origin` (`human` \| `automation` \| `scheduler` \| `driver`). The `origin` column must exist (error if missing); NULL rows are legacy and tolerated (info — read as `human`); a non-NULL value outside the vocabulary is an error (a dispatch path stamped an unknown origin).                                                                                                                                                                                                                                        | `standard-event-store` |
| `runs-db-schema-current`           | warn       | Actual `runs` table columns include every entry in `RUNS_EXPECTED_COLUMNS` (`scripts/runs-db-init.mjs`). A non-origin column going missing (e.g. `hooks_fired_at`) warns here rather than under `run-origin-missing`, so generic schema drift isn't mislabeled as origin-specific. Mirrors `events-db-schema-current`.                                                                                                                                                                                                                                  | `standard-event-store` |
...
| id                        | severity  | what it enforces                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | source standard        |
| ------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `git-sync-gap`            | info      | For every `kind: repo` entity with a `remote_url` and no `sync_policy: fork`, local `default_branch` HEAD matches `origin/<default_branch>` HEAD (via `git ls-remote`, no fetch). Catches the local clone drifting after a GitHub-side merge.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `archetype-entity`     |
| `git-upstream-unreviewed` | info      | The fork-mode counterpart: on a `sync_policy: fork` entity, `origin/<default_branch>` has not moved past the `upstream_reviewed_sha` stamp. Local being ahead of origin is the intentional divergence and never fires. Hint is review-and-stamp (fetch, `git log <reviewed>..origin/<branch>`, re-implement what fits locally, re-stamp) — never merge/rebase/pull.                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `archetype-entity`     |
...
| `supervision-stale`       | info/warn | Per supervision host, `.claude/state/supervision-heartbeat.json` was stamped within that host's window — `scheduler-tick`: 10 minutes, `api-server`: 15 minutes. Both hosts stamp after each completed supervision pass (`stampSupervisionHeartbeat`, `scripts/runs-supervisor.mjs`); a source that stops stamping means dead runs are no longer being finalized and overdue children are no longer being killed. A source that has NEVER stamped is silent (the LaunchAgent may not be installed; the dashboard may never have run here) — only a source that went quiet counts. `warn` severity, with an escalated message, when runs are currently `running`; `info` when nothing is in flight (a quiet host with nothing to supervise is normal). A missing heartbeat file with runs in flight is its own `warn`. | `standard-event-store` |
...
| `events-project-attribution-missing` | warn     | Events whose `skill` is one of the project-scoped skills (`meta-research-project`, `meta-review-project-plan`, `meta-revise-project-plan`, `meta-scaffold-project-plan`) MUST have `project` set. Mirrors `events-skill-attribution-missing` for the project axis, exclusions included. | `standard-event-store`      |
