...
| ------------------ | ----------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| any                | ≠ 0, environment death  | any                                               | **park** — `env-failure: <step> died on <signature> …` (takes precedence over skill-failure — see § Pause reasons)                                                                                                                                                                                                                                                                                                    |
...
| `pr-review`        | 0                       | `pending` / `approved` / `ready-for-human` / null | **complete** — orchestrator sets `pr_review_status: pending → approved` (loop-state: review clean, human triage + Mark ready pending), but only when the linked review's latest pass has no standing blocker/bug comment — a standing-bug `pending` stays `pending`. It no longer flips `ready-for-human` or fires `dashboard.mark-pr-ready` — that is exclusively the human's Mark-ready action (dev-mark-pr-ready). |
...
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `env-failure: <step> died on <signature> (resets <time>) — …`                 | A non-zero run whose evidence names an **environment** condition rather than a skill result: the run row's `error` carries a `model-unavailable(<class>)` line, or the journal tail carries a session-limit banner (reset time extracted when present) or an API overload. Classified by `classifyEnvironmentFailure` (scripts/model-error-policy.mjs). Split out from `skill-failure` because park reasons feed per-skill quality metrics and an infrastructure death is not a verdict on the skill |
| `skill-failure: <step> exited <exit>`                                         | Any other orchestrator-dispatched run that exits non-zero                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `needs-triage: latest pr-review pass has comments to triage …`                | `pr-review` returned `needs-changes` but zero comments on the latest pass are curated for re-implementation (all still `status: new`). Dispatching `address-comments` would no-op; the user triages (accept/dismiss) then resumes (Task #427)                                                                                                                                                                                                                                                        |
| `unknown-step: '<step>' — orchestrator vocabulary out of sync`                | `current_step` value isn't in the canonical v1 set (forward-compat)                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `dispatch-failure: <error>`                                                   | `startRun` returned an error — an in-flight run for the same change, the re-review **debounce** (`⊘ Re-review debounced …`), or the **dirty-tree** gate (`dirty-tree: cannot dispatch …`), or the **branch** gate (`wrong-branch: cannot dispatch …`). Guard-refusal parks persist until a human gesture (they never auto-reconcile — see § Park reconciliation) so the message functions as the operator's cue                                                                                      |
...
## Entry derivation (Start boundary)

The tick-forward advance is artifact-aware (§ Transition rules), but so is the **entry** now. When `POST /start` runs with `current_step: null` it no longer unconditionally dispatches `execute`. Instead `deriveCompletedStepFromArtifacts(change_status, observed, latest_pass_acted)` classifies the highest lifecycle step whose postcondition artifacts already exist:

| artifacts present                                                         | completed step derived                                      | next dispatch (via the reused table)                                                             |
| ------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| linked pr-review with ≥1 pass, comments still open                        | `pr-review`                                                 | verdict tier (address-comments / complete / park)                                                |
| linked pr-review, all curated comments acted-on                           | `address-comments`                                          | `pr-review` (re-review)                                                                          |
| `pr_url` set, no pass yet                                                 | `open-pr`                                                   | `pr-review`                                                                                      |
| branch ref exists AND `status ≠ planning`                                 | `execute`                                                   | `open-pr`                                                                                        |
| degraded git read                                                         | trust `status` (`in-progress`→execute, `in-review`→open-pr) | as if that step completed                                                                        |
| nothing (ref-not-found, no PR, no passes) OR branch at `status: planning` | `null`                                                      | `execute` (genuinely fresh — or a wall-capped EXECUTE that committed but never wrote its status) |

`completed === null` dispatches `execute` (today's behavior for fresh changes); otherwise the SAME `decideNextChangeStep` table produces the next step (`artifact_moved: true` — the classifier just proved the postcondition). This fixes the double-dispatch where a change with completed execute/open-pr artifacts got a redundant `execute`. The `/enable` gesture derives the same entry step for its clean-tree check.

## Park reconciliation

The orchestrator's boundaries reconcile against **out-of-band** completion (the operator hand-finishing a parked step, or a change merging outside the loop). On any automation read/tick/gesture for a stale block, `decideParkReconciliation` runs STATE-ONLY (never dispatches — the artifact-aware Start does that on the next gesture) and can:

- **`complete-terminal`** → a merged/abandoned change with a live `paused`/`running` block + a `current_step` cleans up to `complete`, re-emitting `change-automation-complete` (restores the on-complete audit trigger a stuck park suppressed). Deliberately narrow to `phase ∈ {paused, running}`.
- **`unpark`** → a park whose step completed out-of-band clears to `idle` (keeps `current_step` + `dispatch_baseline` so the next Start advances PAST it; nulls `last_run_id`), emitting `change-automation-reconciled`.

The **unpark bar is a conjunction**: `evaluateArtifactMovement === true` (something landed since this park's own dispatch) AND the parked step's postcondition holds (`deriveCompletedStepFromArtifacts` rank ≥ the parked step's rank). Movement alone would promote a wall-capped EXECUTE that committed but never wrote its status; the postcondition alone would fire on a re-review park's prior pass. Both are required.

Which `paused_reason` prefixes auto-reconcile:

| auto-reconciles (both bars)                     | never auto-reconciles      | why it's excluded                                                                                                                                                     |
| ----------------------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `skill-failure`, `skill-refused`, `env-failure` | `user-paused`              | explicit human stop — never overridden                                                                                                                                |
|                                                 | `needs-triage`             | waits on comment triage; its step artifact already moved → would flip-flop                                                                                            |
|                                                 | `iteration-cap-reached`    | documented Reset/Resume recovery                                                                                                                                      |
|                                                 | `verification-unavailable` | degraded baseline — documented Reset recovery                                                                                                                         |
|                                                 | `dispatch-failure`         | `current_step` is the completed PREVIOUS step, so both bars hold by construction — auto-unpark would erase the debounce/dirty-tree refusal that IS the operator's cue |

Hook sites: `GET /automation`, `/start`, `/resume`, and the 60s merge-watcher walk (folded into the existing wiki walk — no new timer).

## Dispatch guards

Three server-side guards refuse a dispatch cheaply before spawning a paid run — all fail OPEN on any unknown (the in-skill gates are the precise backstops):

- **Re-review debounce** (`evaluatePrReviewDebounce`, enforced in `startRun`): a `dev-pr-review` dispatch whose target branch head equals the last-reviewed pass's `last_head_sha` refuses with `⊘ Re-review debounced …`. Both dispatch paths (orchestrator + dashboard Run-review) converge on `startRun`, so one placement covers both. `force: true` (structured input or a prompt sniff) bypasses; `POST /api/runs` maps a refusal to HTTP 409 `{ error, refusal: 'head-unchanged' }` and the dashboard offers a forced retry. Compares the LOCAL head — the unpushed-commit divergence passes this gate and is caught by dev-pr-review's own head_sha gate.
- **Clean-tree gate** (`TREE_WRITING_STEPS` + `composeDirtyTreeRefusal`, in `dispatchChangeStep` + `/enable`): a tree-writing dispatch (`execute` / `address-comments`) against a dirty clone refuses with `dirty-tree: cannot dispatch …` + the capped file list, instead of burning a run to learn what `git status --porcelain` says (dev-write-change aborts pre-branch anyway).
- **Branch gate** (`evaluateDispatchBranch` + `composeBranchMismatchRefusal`, in `dispatchChangeStep`): the clean-tree gate's companion — a tree-writing dispatch from the wrong checkout refuses with `wrong-branch: cannot dispatch …`. Expected branch is the change's own `branch:` when that ref already exists (resumed work commits onto it) and the repo entity's `default_branch` when it doesn't (the branch is about to be cut from there). A degraded git read, a detached HEAD, or a repo entity with no `default_branch` all fail open. Enforced at dispatch time only; the `/enable` arm-time gate stays tree-only, since a wrong checkout at arm time is routinely corrected before the first Start.
