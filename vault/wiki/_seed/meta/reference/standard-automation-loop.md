...
| `dispatch-failure: <error>`                                                   | `startRun` returned an error — an in-flight run for the same change, the re-review **debounce** (`⊘ Re-review debounced …`), or the **dirty-tree** gate (`dirty-tree: cannot dispatch …`). Guard-refusal parks persist until a human gesture (they never auto-reconcile — see § Park reconciliation) so the message functions as the operator's cue |
...
| -------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `skill-failure`, `skill-refused` | `user-paused`              | explicit human stop — never overridden                                                                                                                                |
...
Two server-side guards refuse a dispatch cheaply before spawning a paid run — both fail OPEN on any unknown (the in-skill gates are the precise backstops):
